...
document.getElementsByTagName('textarea')[0].addEventListener("keyup", () => {
  resizeTextArea();
});

function resizeTextArea() {
  document.getElementsByTagName('textarea')[0].style.height = calcHeight(document.getElementsByTagName('textarea')[0].value) + "px";
}
...
function sendLocalMsg() {
  const msg = document.getElementsByTagName('textarea')[0].value.replace(/\n/g, '<br/>');;
  console.log(msg)
  publishMessage(msg,function(){
    addLocalMsg(msg);
    document.getElementsByTagName('textarea')[0].value = ""; // after the message is sent clear the text area.
    resizeTextArea();
  })
}